...
const SetupItem = ({
  ...
  return (
    <Box borderRadius="200" background={expanded && "bg-surface-active"}>
      <div className={` +
  '`${styles.setupItem} ${expanded ? styles.setupItemExpanded : ""}`' +
  `}>
                  ...
                  <BlockStack gap="400">
                    ...
                    {primaryAction || secondaryAction ? (
                      <ButtonGroup gap="loose">
                        {primaryAction ? (
                          <Button variant="primary" url={primaryAction.url} external>
                            {primaryAction.content}
                          </Button>
                        ) : null}
                        {secondaryAction ? (
                          <Button variant="tertiary" url={secondaryAction.url} external>
...
// SVG code included here for simplicity, you can break out into it's own file
const outlineSvg = (
